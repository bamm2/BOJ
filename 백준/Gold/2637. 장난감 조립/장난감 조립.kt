...
class BOJ2637 {
    ...
    fun run() {
        ...
        val q = ArrayDeque<Product>()
        q.addLast(Product(N - 1, 1))
        count[N - 1] = 1

        while (q.isNotEmpty()) {
            val curr = q.removeFirst()
            for (next in connections[curr.number]) {
                count[next.number] += count[curr.number] * next.cnt
                if (--inDegree[next.number] == 0) q.addLast(Product(next.number, count[next.number]))
            }
        }
